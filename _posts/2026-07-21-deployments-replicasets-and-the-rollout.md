...
── on `kubectl set image` (template changes) ──────────────────────────────

Deployment ──creates a NEW ReplicaSet for the new template──▶ RS "...-9f2a1"
        scales OLD RS down, NEW RS up, in steps bounded by
        maxUnavailable / maxSurge, until only the new RS remains at full size
```
