...
When an event chain spans multiple services (OrderPlaced → FulfillmentStarted → ItemShipped), you need a **correlation ID** (or causation ID) on each event to trace the chain back to the original request. Without it, debugging a failed order across five services is guesswork.
...
**Event versioning.** When you evolve an event schema (e.g., adding a field to OrderPlaced), old consumers must still read old versions. Use a version field in the event type or a schema registry to manage this.
...
**Why Kafka and not a simple queue?** Kafka is a durable, replayable log: topics retain events so new consumers can replay history and rebuild state, which is exactly what projections and the outbox relay need. A queue typically deletes a message after it's consumed, so you can't rebuild a read model from it. Kafka guarantees ordering within a partition, not across the whole topic. If you need global ordering for an entity, all events for that entity must go to the same partition (same key).
