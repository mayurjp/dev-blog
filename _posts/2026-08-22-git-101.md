...
The mental model that matters is this: Git is not a folder of file versions. It is a **content-addressed store of objects** (blobs, trees, commits, tags) addressed by SHA-1, wired together into a **DAG** by commit parent pointers, with a thin layer of **refs** (branch and tag names) pointing into that graph. Learn those three ideas and every Git command is just an operation on them.
...
The two layers are deliberately separate, and that separation is the whole game:
