...
### Q: Why does changing one source file sometimes cause a 10-minute rebuild instead of a 10-second one?
...
→ Post: `_posts/2025-07-30-docker-image-layers-and-build-cache.md`

### Q: What is the difference between a metadata instruction like `CMD` and a filesystem instruction like `RUN`?
...
### Q: Why do two images built from the same base take less disk space than building them independently?
...
### Q: What component actually manages the layer content store and the overlay2 snapshotter — dockerd or containerd?
...
### Q: What's the common mistake when writing a Dockerfile that destroys build cache?
...
### Q: How does `RUN --mount=type=cache` differ from regular layer caching?
...
### Q: What is the build context, and why does a stray `node_modules/` or `.git/` directory matter?
...
→ Post: `_posts/2025-08-01-dockerfile-instructions-and-build-context.md`

### Q: What's the practical difference between `ARG` and `ENV` in a Dockerfile?
...
### Q: What does `EXPOSE 3000` actually do at runtime?
...
### Q: What is the `# syntax=docker/dockerfile:1.7` directive at the top of a Dockerfile for?
...
### Q: What happens to the build if you use shell-form `CMD` in an image with no shell (e.g., `FROM scratch`)?
...
### Q: Why does the production awesome-compose Dockerfile use four separate `ENV` instructions instead of one combined line?
...
### Q: Why does a single-stage Dockerfile that compiles code produce an image hundreds of megabytes larger than necessary?
...
→ Post: `_posts/2025-08-07-multi-stage-builds-shrinking-the-final-image.md`

### Q: How does `docker build --target=builder` change what gets shipped?
...
### Q: Does a later stage automatically inherit the filesystem of an earlier stage?
...
### Q: What is the trade-off between Alpine, Ubuntu, and distroless base images in Grafana's multi-variant build?
...
### Q: Why does Grafana use bare, seemingly unused `FROM` lines at the top of its Dockerfile?
...
### Q: How does `COPY --link` improve build performance in a wide multi-stage graph?
...
### Q: Why doesn't `runc` stay running as a child process after it starts a container?
...
→ Post: `_posts/2025-08-13-containers-runtime-containerd-runc.md`

### Q: What is the actual role of `containerd-shim-runc-v2` in the runtime chain?
...
### Q: Why does `docker stop` send `SIGTERM` to PID 1, and why does exec-form `CMD` matter for signal handling?
...
### Q: What happens if PID 1 in a container is a process that spawns children but never calls `wait()`?
...
### Q: What's the difference between `dockerd`'s responsibilities and `containerd`'s in modern Docker Engine?
...
### Q: What does `FROM scratch` actually mean, and what does it tell you about what the container runtime requires?
...
### Q: What happens to data written to a container's filesystem when the container is removed?
...
→ Post: `_posts/2025-08-19-volumes-and-bind-mounts.md`

### Q: How does a named volume differ from a bind mount in terms of host-path management?
...
### Q: What mistake does Compose make if you reference a volume name without declaring it under the top-level `volumes:` key?
...
### Q: When would you use `read_only: true` on a bind mount?
...
### Q: Why does the Sentry stack mark its named volumes with `external: true`?
...
### Q: Can a named volume and a bind mount be used on the same service? What would that look like?
...
### Q: Why can't containers on the default `bridge` network resolve each other by name?
...
→ Post: `_posts/2025-08-25-docker-networking-bridge-and-service-discovery.md`

### Q: What happens to name resolution when a container restarts with a new IP on a user-defined network?
...
### Q: How does Traefik discover which containers to route traffic to?
...
### Q: What's the risk of not setting `--providers.docker.exposedbydefault=false` on Traefik?
...
### Q: How do overlay networks extend the same name-based discovery model across multiple Docker hosts?
...
### Q: Why doesn't the Traefik compose file declare an explicit `networks:` block despite having multiple services?
...
### Q: What's the difference between `condition: service_started` and `condition: service_healthy` in `depends_on`?
...
→ Post: `_posts/2025-08-31-docker-compose-multi-container-orchestration.md`

### Q: Why does Sentry's compose file mix `*depends_on-healthy` and `*depends_on-default` on the same service?
...
### Q: How do YAML anchors and `x-` top-level keys make a 70-service compose file maintainable?
...
### Q: What does environment variable interpolation like `$HEALTHCHECK_INTERVAL` in a compose file actually do?
...
### Q: Why is the legacy `docker-compose` (hyphenated, V1) different from `docker compose` (V2)?
...
### Q: What happens if you don't declare a `networks:` block in a multi-service compose file?
...
### Q: Why is a healthcheck probe's test command limited to telling you about the container's internal state, not external reachability?
...
→ Post: `_posts/2025-09-06-healthchecks-and-restart-policies.md`

### Q: What happens if a container is marked `unhealthy` — does Docker restart it?
...
### Q: What does `start_period` do in a healthcheck, and why is omitting it dangerous?
...
### Q: How does `restart: true` inside a `depends_on` entry differ from the top-level `restart:` policy?
...
### Q: Why does each service in Sentry's stack have a protocol-specific healthcheck test instead of a generic template?
...
### Q: What does the `x-file-healthcheck` anchor in Sentry's compose file do, and why is it written as inline Python?
...
### Q: Why can one runaway container take down every other container on the host if no resource limits are set?
...
→ Post: `_posts/2025-10-06-docker-resource-limits-and-cgroups.md`
...
### Q: What's the trap with `deploy.resources` in a Compose file for resource limits?
...
### Q: Why does Elasticsearch use `ulimits.memlock: {soft: -1, hard: -1}` in addition to `mem_limit`?
...
### Q: Why do some teams set memory limits on every service but never set CPU limits?
...
### Q: What underlying kernel mechanism do Docker's `mem_limit` and `cpus` settings map to?
...
### Q: Why is the second `docker push` of a rebuilt image dramatically faster than the first?
...
→ Post: `_posts/2025-10-08-registries-and-image-distribution.md`

### Q: What's the relationship between a tag, a manifest, and a blob in a registry?
...
### Q: How does a single tag like `myapp:1.0` serve different binaries on `linux/amd64` vs `linux/arm64`?
...
### Q: Why does `:latest` have no special meaning in the registry protocol?
...
### Q: What does `storage.cache.blobdescriptor: inmemory` in the distribution registry config actually cache?
...
### Q: What's the growing problem with `FROM` lines that default to Docker Hub for base images?
...
### Q: Why does `docker build --no-cache` still skip redownloading packages when using BuildKit cache mounts?
...
→ Post: `_posts/2025-10-10-buildkit-cache-mounts-vs-layer-caching.md`

### Q: Why does Grafana use two separate cache mounts (`/go/pkg/mod` and `/root/.cache/go-build`) on the same `RUN` instruction instead of one?
...
### Q: How do layer caching and BuildKit cache mounts solve complementary problems?
...
### Q: What does `COPY --parents **/go.mod **/go.sum ./` do, and why is it needed in Grafana's build?
...
### Q: Why are cache mount contents "invisible to the final image"?
...
### Q: Is `DOCKER_BUILDKIT=1` still necessary to use cache mounts and BuildKit features?
...
### Q: Why doesn't `USER 1000` in a Dockerfile alone make a container secure?
...
→ Post: `_posts/2025-10-12-docker-security-nonroot-capabilities-seccomp.md`

### Q: What does Docker's default seccomp profile actually block?
...
### Q: What's the difference between rootless Docker and running a container as non-root?
...
### Q: Why does Grafana's distroless Dockerfile use `printf` to write `/etc/passwd` instead of `useradd`?
...
### Q: Why does Grafana set GID to 0 (root's group) and `chmod 777` on data directories?
...
### Q: Which security layer runs first in the kernel when a process attempts an action — seccomp or capabilities?
...
### Q: Why does `docker buildx build --platform linux/amd64,linux/arm64` take 10x longer than a single-platform build on an x86_64 runner?
...
→ Post: `_posts/2025-10-14-multi-arch-builds-buildx-emulation-vs-cross-compile.md`

### Q: How does Traefik avoid QEMU emulation entirely in its multi-arch build?
...
### Q: How does the Dockerfile know which pre-built binary to copy for each platform?
...
### Q: Why can't the default `docker` builder driver produce multi-platform output?
...
### Q: Why does Traefik's GitHub Actions workflow install QEMU even though its Dockerfile never needs it?
...
### Q: Is the cross-compile-then-COPY pattern universal, or only for specific languages?
