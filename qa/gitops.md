---
layout: page
title: "GitOps — Q&A Bank"
permalink: /qa/gitops/
---

Bite-sized questions and answers from GitOps blog posts. Read 5-10 per sitting. Each answer is 2-4 sentences max and links back to the full post for deeper understanding.

## Topic: Pull-based deployment & the reconciliation loop (Order 1)

### Q: What problem does pull-based GitOps solve that push-based CD does not?
...
→ Post: `_posts/2026-06-07-gitops-pull-based-reconciliation-loop.md`

### Q: How does Argo CD decide *when* to reconcile an Application — is it purely time-based polling?
...
### Q: What is the "live side" of the diff in Argo CD's reconcile loop?
...
### Q: What happens if Argo CD loses connectivity to the cluster's API server?
...
### Q: Why is holding cluster credentials in a CI pipeline a security concern in GitOps?
...
### Q: What's the difference between `sync.Reconcile` and `argodiff.StateDiffs` in Argo CD?
...
### Q: How does the `syncNeeded` flag relate to the hard-expiry fallback in `needRefreshAppStatus`?
...
### Q: Why does a `kubectl edit` on a label injected by a mutating webhook survive in a GitOps-managed cluster?
...
→ Post: `_posts/2026-06-23-declarative-desired-state-three-way-merge-drift.md`

### Q: What are the three inputs to GitOps drift detection, and where does each come from?
...
### Q: What happens if the `last-applied-configuration` annotation is manually deleted from a live resource?
...
### Q: How does `TwoWayDiff` work internally in `argoproj/gitops-engine`?
...
### Q: Why is this three-way merge mechanism more significant for GitOps than for a one-off `kubectl apply`?
...
### Q: When you intentionally remove a field from a Git manifest, does the three-way merge remove it from the live object?
...
### Q: How does Server-Side Apply (SSA) differ from the annotation-based three-way merge in drift detection?
...
### Q: What determines the execution order of resources in Argo CD's sync engine?
...
→ Post: `_posts/2026-07-07-gitops-sync-waves-hooks-wave-gating.md`

### Q: How does the sync gate prevent the next wave from starting?
...
### Q: Do sync waves apply to hooks like PreSync and PostSync?
...
### Q: What happens if two resources in the same wave have a dependency on each other?
...
### Q: When is the sync gate behavior skipped entirely?
...
### Q: Why is `kindOrder` important even when no explicit wave annotations are used?
...
### Q: Should wave numbers be assigned as 0, 1, 2 or with gaps like 0, 10, 20?
...
### Q: What prevents Argo CD's auto-sync from creating an infinite sync loop?
...
→ Post: `_posts/2026-07-23-argo-cd-auto-sync-self-heal-pruning.md`

### Q: When auto-sync is enabled but pruning is disabled, what happens if the only change is a resource removal?
...
### Q: Does self-heal sync the entire application or only the drifted resources?
...
### Q: What is the "wipe-out guard" in Argo CD's auto-sync?
...
### Q: What are the five guards in the `autoSync` function before a sync operation is created?
...
### Q: Why does self-heal only trigger when the Git revision hasn't changed?
...
### Q: What are the `*bool` pointer fields in `SyncPolicyAutomated`, and why are they pointers?
...
### Q: What problem do ApplicationSets solve that manually creating Application resources does not?
...
→ Post: `_posts/2026-07-23-argo-cd-applicationsets-app-of-apps.md`

### Q: How does the matrix generator differ from the merge generator?
...
### Q: What happens when an ApplicationSet generator returns zero results?
...
### Q: How does the ApplicationSet controller handle deletion of the ApplicationSet resource itself?
...
### Q: How does `firstAppError` handle concurrent failures in ApplicationSet reconciliation?
...
### Q: What annotations are preserved across Application updates by the ApplicationSet controller?
...
### Q: What does `helm rollback myapp 3` actually restore?
...
→ Post: `_posts/2026-07-23-helm-release-history-rollback-revision.md`

### Q: What's the difference between `Superseded` and `Failed` in Helm release history?
...
### Q: What happens if you run `helm rollback myapp` without specifying a revision number?
...
### Q: Does `helm rollback` re-render the chart from the original template or use the stored manifest?
...
### Q: How does `RollbackOnFailure` in `helm upgrade` find its target?
...
### Q: How are Helm release revisions stored in Kubernetes?
...
### Q: What happens to existing revisions when you rollback from revision 7 to revision 3?
...
### Q: Why do Kustomize overlays produce cleaner diffs than Helm template conditionals?
...
→ Post: `_posts/2026-07-23-kustomize-overlays-vs-helm-templates.md`

### Q: How does Kustomize's `PatchTransformerPlugin` prevent ambiguous patch formats?
...
### Q: Why does `kustomize build` need OpenAPI schema to apply patches correctly?
...
### Q: Can Kustomize and Helm coexist in the same pipeline?
...
### Q: Why is the `applySortOrder` pass important in `kustomize build`?
...
### Q: What's the performance tradeoff of Kustomize's overlay DAG vs. Helm's flat values merge?
...
### Q: How does Flux's `ImageUpdateAutomation` know which image field to rewrite?
...
→ Post: `_posts/2026-07-23-flux-image-automation-progressive-delivery.md`

### Q: What prevents the `ImageUpdateAutomation` controller from creating empty commits?
...
### Q: How does Flux's image automation differ from Renovate or Dependabot for container tags?
...
### Q: How does the controller handle a failed Git push due to branch protection rules?
...
### Q: How does the `commitTemplate` work for batch image updates?
...
### Q: Why does the controller use a `latestImageChangePredicate` on the ImagePolicy watch instead of reacting to all status updates?
...
### Q: What happens when two `ImageUpdateAutomation` controllers push to the same branch simultaneously?
...
### Q: How does Argo CD's cluster generator discover clusters at runtime?
...
→ Post: `_posts/2026-07-23-argo-cd-cluster-generator-multi-cluster.md`

### Q: What happens to the local `in-cluster` when a label selector is used?
...
### Q: How does `flatList` mode differ from the standard cluster generator output?
...
### Q: Why is `nameNormalized` a separate parameter from `name`?
...
### Q: How does the cluster generator detect new clusters — is it poll-based?
...
### Q: How does environment promotion work with Argo CD's Git generator?
...
→ Post: `_posts/2026-07-23-argo-cd-git-generator-environment-promotion.md`

### Q: What triggers Argo CD to re-read the Git repo for the file-based generator?
...
### Q: What automatic parameters does the Git file generator inject?
...
### Q: How does the directory-based Git generator differ from the file-based one?
...
### Q: What happens if two environment files have the same name under the glob pattern?
...
### Q: Why is script-driven promotion inferior to Git-commit-driven promotion?
...
### Q: What are the five core problems with push-based CI/CD that Flux's pull-based model solves?
...
→ Post: `_posts/2026-07-23-flux-pull-based-reconciliation-vs-push.md`

### Q: How does Flux's inventory tracking enable pruning?
...
### Q: Why does Flux use server-side apply (SSA) instead of client-side `kubectl apply`?
...
### Q: How does Flux take ownership away from `kubectl` when it applies?
...
### Q: What's the architectural difference between Flux's Source Controller and Kustomization Controller?
...
### Q: What happens to the cluster's state when the Source Controller can't reach Git?
...
### Q: What resource does Flux use to prevent path traversal during Kustomize builds?
...
### Q: Why doesn't External Secrets Operator store secret values in Git?
...
→ Post: `_posts/2026-07-23-external-secrets-operator-gitops-bridge.md`

### Q: What does an `ExternalSecret` CRD actually contain?
...
### Q: How does `refreshInterval` bound the latency of credential rotation?
...
### Q: What's the difference between `CreatedOnce`, `OnChange`, and `Periodic` refresh policies?
...
### Q: What happens to the Kubernetes Secret if the external store is temporarily unreachable?
...
### Q: Why does the AWS Parameter Store provider use `gjson` for property extraction?
...
### Q: What is the blast-radius difference between `SecretStore` and `ClusterSecretStore`?
...
### Q: Why did Flux v2.6 break teams' drift alerts?
...
→ Post: `_posts/2026-07-23-flux-reconciled-status-metrics-drift-alerting.md`

### Q: What are the three status conditions on a Kustomization CRD that matter for drift alerting?
...
### Q: Why should drift alerts use `for:` durations that match the reconciliation interval?
...
### Q: How do you surface Flux condition data to Prometheus after v2.6?
...
### Q: What is the difference between `Ready=False` and `Reconciling=True`?
...
### Q: Why did Flux move conditions from controller-emitted Prometheus gauges to Kubernetes status conditions?
...
### Q: What does it mean when `lastAttemptedRevision` and `lastAppliedRevision` diverge?
