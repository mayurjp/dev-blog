---
layout: page
title: "MLOps — Q&A Bank"
permalink: /qa/mlops/
---

Bite-sized questions and answers from MLOps blog posts. Read 5-10 per sitting. Each answer is 2-4 sentences max and links back to the full post for deeper understanding.

## Topic: ML pipeline fundamentals (Order 1)

### Q: What structural problem does a DAG-of-containers pipeline solve that `jupyter nbconvert --execute` on a schedule cannot?
...
→ Post: `_posts/2026-05-22-ml-pipeline-fundamentals-dag-isolation-and-caching.md`

### Q: How does Kubeflow's per-step caching differ from simply not re-running a notebook cell?
...
### Q: What happens if you bump a pipeline step's base image but don't change its data inputs?
...
### Q: Why are PVC names folded into the cache fingerprint instead of being ignored?
...
### Q: What is the tradeoff of per-step containerization compared to running everything in a single notebook environment?
...
### Q: What's the common mistake when defining DAG edges in Kubeflow's Python SDK?
...
### Q: What is point-in-time correctness, and why can't a naive join achieve it?
...
→ Post: `_posts/2026-06-17-feature-stores-point-in-time-join-vs-latest-lookup.md`

### Q: How does `get_online_features` differ from `get_historical_features` at the query-structure level?
...
### Q: What happens if a feature view's TTL is set too short for a slowly-changing feature?
...
### Q: What is the tradeoff of using a feature store's point-in-time join versus computing features inline in the training script?
...
### Q: Why does `get_online_features` not need a TTL filter even though features can be stale?
...
### Q: What's the common mistake when building a training dataset that bypasses the feature store?
...
### Q: How does MLflow capture the git commit for a run without any user logging code?
...
→ Post: `_posts/2026-07-01-experiment-tracking-git-context-run-lineage.md`

### Q: How does MLflow's `runs:/<run_id>/<artifact_path>` URI scheme differ from saving a model to an arbitrary path?
...
### Q: What happens if training code runs outside a git repository when MLflow tracking is enabled?
...
### Q: What is the limitation of commit-based lineage for notebook-driven training?
...
### Q: What's the common mistake when logging a trained model that breaks run lineage?
...
### Q: How does the provider registry pattern in MLflow's context system differ from a hardcoded git-commit capture?
...
### Q: How does DVC keep large data files out of git while still tracking them in version control?
...
→ Post: `_posts/2026-07-15-data-versioning-dvc-content-addressed-cache.md`

### Q: How does DVC's content-addressed cache differ from Git LFS?
...
### Q: What happens if the local DVC cache is deleted and no remote has been configured?
...
### Q: Why must files in DVC's content-addressed cache be read-only (mode 0o444)?
...
### Q: What's the tradeoff of using MD5 as DVC's content hash?
...
### Q: What happens if a data file is renamed outside of DVC's own commands?
...
### Q: What problem do promotion stages solve that version numbers alone cannot?
...
→ Post: `_posts/2026-07-29-mlflow-model-registry-promotion-stages.md`

### Q: How do model version aliases (MLflow 2.9+) differ from the fixed-stage lifecycle?
...
### Q: What happens if you promote a version to Production without setting `archive_existing_versions=True`?
...
### Q: Why is the stage transition atomic (single DB transaction) instead of two separate updates?
...
### Q: What's the common mistake when promoting to None or Archived?
...
### Q: What happens if you promote a Production version back to Staging?
...
### Q: Why can't a batch scoring pipeline serve a prediction API with sub-100ms latency?
...
→ Post: `_posts/2026-07-29-kserve-model-serving-batch-vs-online.md`

### Q: How does KServe's transformer/predictor split differ from a monolithic model server?
...
### Q: What happens if you forget to set `self.ready = True` in a KServe model's `load()` method?
...
### Q: Why does KServe forward only three headers (`x-request-id`, `x-b3-traceid`, `authorization`) instead of all headers?
...
### Q: What's the downside of KServe's per-stage Prometheus histograms for online serving?
...
### Q: What happens if `preprocess()` is slow in a KServe model?
...
### Q: How does Triton's dynamic batching amortize GPU overhead across concurrent requests?
...
→ Post: `_posts/2026-07-29-triton-dynamic-batching-inference-optimization.md`

### Q: How does Triton's `max_queue_delay_microseconds` differ from `preferred_batch_size` as a batching trigger?
...
### Q: What happens if only one request arrives within the `max_queue_delay` window?
...
### Q: Why is `preserve_ordering` mandatory for autoregressive LLMs but wasteful for stateless classifiers?
...
### Q: What is the latency tax of dynamic batching, and how do you tune it?
...
### Q: What's the common mistake when setting `preferred_batch_size` for a new model?
...
### Q: How does Argo Rollouts' canary traffic shifting differ from a plain Kubernetes rolling update?
...
→ Post: `_posts/2026-07-29-argo-rollouts-ml-model-canary-traffic-shifting.md`

### Q: What is an `AnalysisRun`, and how does it gate model promotion?
...
### Q: What is the tradeoff of `dynamicStableScale: true` for GPU-heavy model rollouts?
...
### Q: Why is `pause.duration` on analysis steps critical for statistical significance?
...
### Q: What happens if the Istio VirtualService doesn't exist when the Rollout is created?
...
### Q: Why can't you use only PSI or only KS for production drift detection?
...
→ Post: `_posts/2026-07-29-evidently-data-drift-distribution-shift.md`

### Q: How does PSI's bin-based approach differ from KS's CDF-based approach mathematically?
...
### Q: What happens if you try to apply the KS test to a categorical column in Evidently?
...
### Q: Why does KS become unreliable with very large sample sizes?
...
### Q: Why can't you directly compare PSI and KS scores?
...
### Q: What's the common mistake when choosing a reference window for drift detection?
...
### Q: Why do Prometheus's default histogram buckets produce inaccurate p99 percentiles for ML inference latency?
...
→ Post: `_posts/2026-07-29-prometheus-ml-monitoring-latency-histograms.md`

### Q: How do Prometheus native histograms differ from classic histograms in bucket boundary storage?
...
### Q: What happens if you change classic histogram bucket boundaries on an existing metric?
...
### Q: Why can't you use summaries instead of histograms for cluster-wide p99 percentiles?
...
### Q: What is the memory cost of custom histogram buckets?
...
### Q: What happens if an inference service produces a 0.0s observation when using custom bucket native histograms?
...
### Q: How does TFDS avoid re-downloading and re-transforming data on every `tfds.load()` call?
...
→ Post: `_posts/2026-07-29-tfds-feature-engineering-data-validation.md`

### Q: How does `DatasetInfo` track cache validity, and what invalidates it?
...
### Q: What happens if you change the feature normalization in `_info()` without bumping the version?
...
### Q: What's the downside of using TFRecord as the default cache format?
...
### Q: What happens if two scripts call `tfds.load()` for the same dataset simultaneously?
...
### Q: What's the common mistake when relying on TFDS's auto-short-circuit in CI?
...
### Q: Why does Great Expectations treat empty strings (`""`) as different from NULL?
...
→ Post: `_posts/2026-07-29-great-expectations-ml-data-contracts.md`

### Q: How does the `mostly` parameter differ from strict null checks?
...
### Q: What happens when a new category appears in a categorical feature that's gated by a data contract?
...
### Q: Why should you not use Great Expectations' auto-profiler for production data contracts?
...
### Q: How does the `ValidationGraph` prevent redundant metric computation across expectations?
...
### Q: What's the common mistake when running Great Expectations in CI?
...
### Q: How does TPE find good hyperparameters in fewer trials than grid or random search?
...
→ Post: `_posts/2026-07-29-optuna-tree-parzen-bayesian-optimization.md`

### Q: How does TPE differ from Gaussian Process-based Bayesian optimization?
...
### Q: What is `constant_liar` and when should you use it?
...
### Q: Why is `multivariate=True` critical for hyperparameter optimization?
...
### Q: What is the tradeoff of the gamma function's cap at 25 trials?
...
### Q: What storage backend should you use for parallel Optuna optimization in production?
...
### Q: How does gradient bucketing overlap AllReduce with the backward pass?
...
→ Post: `_posts/2026-07-29-pytorch-ddp-gradient-bucketing-allreduce.md`

### Q: How does DDP's communication strategy differ from FSDP's?
...
### Q: What is the tradeoff of `gradient_as_bucket_view=True`?
...
### Q: Why are bucket indices reversed at Reducer creation?
...
### Q: What's the common mistake when using `find_unused_parameters=True`?
...
### Q: Why must Airflow's retry state live in the metadata database instead of in the worker's memory?
...
→ Post: `_posts/2026-07-29-airflow-dag-executor-taskinstance-retries.md`

### Q: How does Airflow's heartbeat-based liveness detection work?
...
### Q: What happens if a worker crashes between getting a task from the queue and writing `state = RUNNING` to the DB?
...
### Q: Why is `max_tries` cumulative rather than per-clear?
...
### Q: Why does `prepare_db_for_next_try()` assign a new UUID on every retry?
...
### Q: What prevents thundering-herd retries in Airflow when multiple tasks fail simultaneously?
...
### Q: Why does the worker ignore SIGINT but handle SIGTERM?
