...
### Q: What problem does multi-cloud solve, and why isn't "avoiding lock-in" usually a sufficient driver?
...
→ Post: `_posts/2026-06-11-multi-cloud-why-really-terraform-schema-divergence.md`

### Q: How does AWS RDS's `multi_az` field differ structurally from GCP Cloud SQL's `availability_type`?
...
### Q: What is the ongoing engineering cost of multi-cloud that most tutorials skip?
...
### Q: Can a well-written Terraform module eliminate the schema divergence between AWS and GCP resources?
...
### Q: What happens when a team assumes `backup_retention_period` on AWS and `retained_backups` on GCP are interchangeable?
...
### Q: What problem does Terraform's provider protocol solve for multi-cloud?
...
→ Post: `_posts/2026-06-25-terraform-provider-rpc-protocol-schema-agnostic-core.md`

### Q: How does Terraform Core avoid compile-time coupling to any specific cloud's resource schema?
...
### Q: What makes it possible for Terraform to support 3,000+ providers without an equally large switch statement in Core?
...
### Q: Why is each Terraform provider a separate OS process rather than a library linked into Core?
...
### Q: When debugging a plan/apply discrepancy for a specific resource, is the bug more likely in Terraform Core or in the provider?
...
### Q: How does Terraform handle state upgrades when a provider changes a resource's schema?
...
### Q: Why doesn't a Kubernetes ServiceAccount granting cloud IAM permissions work identically on GKE and EKS?
...
→ Post: `_posts/2026-07-09-kubernetes-multicloud-abstraction-workload-identity-vs-irsa.md`

### Q: What per-cluster prerequisite does AWS IRSA require that GKE Workload Identity does not?
...
### Q: What happens if the OIDC provider ARN in an IRSA trust policy doesn't match the actual cluster's issuer URL?
...
### Q: How does AWS IRSA's `aud` (audience) condition differ from anything in GKE Workload Identity?
...
### Q: Is the migration of a workload's cloud-IAM binding from EKS to GKE just a matter of changing the annotation value?
...
### Q: Does application code need to know which cloud's identity mechanism is in use?
...
### Q: What is the three-layer object model in Crossplane that bridges a developer's YAML claim to actual cloud resources?
...
→ Post: `_posts/2026-07-25-crossplane-kubernetes-native-multi-cloud-control-plane.md`

### Q: What role does the Composition's gRPC function pipeline play in Crossplane?
...
### Q: Why doesn't updating a Composition instantly affect all running Claims?
...
### Q: How does Crossplane avoid loading watches for every possible cloud resource type at startup?
...
### Q: What does Crossplane's circuit breaker do and when does it trigger?
...
### Q: What is the `BetaFallBackFunctionRunnerServiceClient` and why does it exist?
...
### Q: What happens if a Composition only patches into Managed Resources but never patches back to the Claim?
...
### Q: What are the three distinct mechanisms Consul uses for multi-cloud networking, and what does each handle?
...
→ Post: `_posts/2026-07-25-consul-multi-cloud-networking-vpc-boundaries.md`

### Q: What does the `PeerThroughMeshGateways` boolean in MeshConfigEntry control?
...
### Q: What happens after peering is established but before `exported-services` is configured?
...
### Q: Can a team use both WAN federation and peering for the same pair of datacenters?
...
### Q: What is a `PeeringToken` and what does it contain?
...
### Q: What fails silently if the `exported-services` entry has a typo in the peer name?
...
### Q: What are the three failure modes of storing long-lived AWS access keys as GitHub Actions secrets?
...
→ Post: `_posts/2026-07-25-aws-iam-oidc-identity-provider-terraform.md`

### Q: What must the OIDC provider's `client_id_list` include for GitHub Actions OIDC federation to work?
...
### Q: Why does Terraform retry role creation when using OIDC federation?
...
### Q: What happens if you use a `*` wildcard in the trust policy's `sub` condition in production?
...
### Q: Why is the `thumbprint_list` empty for GitHub's OIDC provider?
...
### Q: Where does the S3 compatibility abstraction break when using MinIO across clouds?
...
→ Post: `_posts/2026-07-25-minio-s3-compatible-cloud-agnostic-storage.md`

### Q: Why does MinIO use "dummy call" handlers (GetBucketACL returning empty 200) instead of returning 401/403?
...
### Q: What does MinIO's `rejectedObjAPIs` and `rejectedBucketAPIs` list represent?
...
### Q: Why is the `ObjectInfo` struct in MinIO not cloud-portable?
...
### Q: What is a gotcha when using MinIO's extension APIs in cloud-agnostic code?
...
### Q: How does CoreDNS resolve multi-cluster service names differently from single-cluster names?
...
→ Post: `_posts/2026-07-25-coredns-plugin-chain-multi-cluster-dns.md`

### Q: What happens if the `fallthrough` directive is omitted from the CoreDNS Corefile for a multi-cluster zone?
...
### Q: Why does plugin ordering in the Corefile matter?
...
### Q: What happens at startup if `multicluster` is set in the Corefile but the MCS API CRDs are not installed?
...
### Q: Why does Istio need east-west gateways for multi-cluster across cloud VPCs?
...
→ Post: `_posts/2026-07-25-istio-multi-cluster-mesh-east-west-gateway.md`

### Q: What does the `EndpointsByNetworkFilter` in Istio's endpoint builder actually do?
...
### Q: What is the `forceGateway` edge case in `EndpointsByNetworkFilter` and why does it matter?
...
### Q: Why does `selectNetworkGateways` prefer gateways in the same cluster as the target endpoints?
...
### Q: How does the ambient (non-sidecar) mode differ from sidecar mode in the gateway endpoint's wire protocol?
...
### Q: What is the common misconfiguration that causes cross-cluster traffic to fail silently?
...
### Q: Why can't AWS Cost Explorer tell you which Kubernetes namespace consumed 400 CPU-core-hours of EC2 spend?
...
→ Post: `_posts/2026-07-25-kubecost-finops-per-namespace-cost-allocation.md`

### Q: How does OpenCost's allocation engine compute per-namespace costs?
...
### Q: Why do OpenCost's Prometheus collectors read from an in-memory `ClusterCache` rather than depending on kube-state-metrics?
...
### Q: What happens if a node's Spot price changes mid-allocation window?
...
### Q: Why does `ComputeAllocation` batch Prometheus queries into windows instead of querying the full range at once?
...
### Q: Why can't you run a single etcd cluster stretched across two clouds for unified multi-cloud state?
...
→ Post: `_posts/2026-07-25-etcd-multi-cluster-raft-consensus-limits.md`

### Q: What does etcd's lessor (lease manager) do that makes a stretched cluster particularly bad?
...
### Q: What is the production pattern for multi-cloud etcd, and what tradeoff does it accept?
...
### Q: Why do cross-cloud network latencies of 10-80 ms cause problems for a stretched etcd cluster even without a full partition?
...
### Q: What is the RPO when using independent etcd clusters with async replication?
...
### Q: Where does Terraform's real lock-in live — in HCL syntax or in the state file?
...
→ Post: `_posts/2026-07-25-terraform-provider-protocol-lock-in-leverage.md`

### Q: What is `Private []byte` in the state file and why is it the deepest lock-in mechanism?
...
### Q: Why does `terraform state pull` on a state file from another team sometimes download providers you don't use?
...
### Q: What is the practical escape hatch for cross-cloud migration in Terraform?
...
### Q: Does OpenTofu's state format differ from Terraform's in ways that affect portability?
...
### Q: What happens if you downgrade a provider version after a schema bump that changed `SchemaVersion`?
