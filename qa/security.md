...
### Q: What problem does JWKS discovery solve that baking keys into verifier config does not?
...
→ Post: `_posts/2026-06-03-jwks-discovery-and-key-rotation.md`

### Q: Why is JWKS rotation an insert, not a replacement, and what happens if you delete the old key too early?
...
### Q: What is the RS256→HS256 key-confusion vulnerability, and how does a verifier prevent it?
...
### Q: Why does `hydra delete jwk <set>` destroy more than intended for key rotation cleanup?
...
### Q: How does Hydra determine which key signs a new token versus which keys are published for verification?
...
### Q: Why must a verifier match tokens by `kid` rather than assuming a single key at a time?
...
### Q: Why is a fast hash like SHA-256 unsafe for password storage but appropriate for API key storage?
...
→ Post: `_posts/2025-09-08-password-hashing-and-credential-storage.md`

### Q: What is the purpose of the salt in password hashing, and why doesn't it need to be secret?
...
### Q: What does `SuccessRehashNeeded` mean in ASP.NET Core's `PasswordHasher`, and when is it returned?
...
### Q: Why is `CryptographicOperations.FixedTimeEquals` used instead of `==` for hash comparison?
...
### Q: How does ASP.NET Core's versioned hash format enable algorithm migration without forced password resets?
...
### Q: Why is PBKDF2 still in ASP.NET Core despite OWASP now recommending Argon2id?
...
### Q: Why can't a client forge identity by editing their own session cookie?
...
→ Post: `_posts/2025-11-11-session-based-authentication.md`

### Q: What is session fixation and why must `cycle_key()` be called on login?
...
### Q: What structural advantage does session-based auth have over stateless JWT tokens for revocation?
...
### Q: Why does Django's `_get_new_session_key` check `self.exists(session_key)` instead of just returning a random string?
...
### Q: Why is the server-side expiry check authoritative rather than the cookie's `Max-Age`?
...
### Q: Is "session-based auth doesn't scale, use JWTs instead" accurate?
...
### Q: Why is the JWT signature computed over both the header and the payload, not just the payload?
...
→ Post: `_posts/2025-11-13-token-based-authentication-jwt-structure.md`

### Q: Why can't a JWT payload be kept confidential just by using a JWT?
...
### Q: What is the `TimeMargin` in JWT expiry checking, and why is a naive `now >= exp` problematic?
...
### Q: What is the key material difference between HMAC-signed and RSA-signed JWTs in terms of trust model?
...
### Q: Why does `header.Add("alg", algorithm.Name)` being inside the signed portion matter for attack defense?
...
### Q: What common mistake does WebGoat's broken access control example illustrate, and how does it relate to JWT algorithm confusion?
...
→ Post: `_posts/2025-12-01-common-authn-authz-vulnerabilities.md`

---

## Topic: OAuth 2.0 authorization code flow (Order 4)

### Q: What problem does PKCE solve that the basic authorization code flow cannot?
...
→ Post: `_posts/2025-11-15-oauth2-authorization-code-flow-and-pkce.md`

### Q: Why is SHA-256 used to compute the code_challenge from the code_verifier?
...
### Q: Why does Ory Hydra enforce a 43-128 character minimum/maximum for the code_verifier?
...
### Q: Why is the `plain` PKCE method considered weaker, and how does Hydra handle it?
...
### Q: Why must the code_challenge be stored tied to the authorization code's signature, not just held in memory?
...
### Q: Is PKCE only for public/mobile clients in current best practices?
...
### Q: What gap does OIDC fill that raw OAuth2 access tokens leave open?
...
→ Post: `_posts/2025-11-17-openid-connect-identity-layer-on-oauth2.md`

### Q: How does the `at_hash` claim prevent token splicing?
...
### Q: Why does OIDC's `nonce` defeat replay attacks but a JWT's `exp` alone does not?
...
### Q: Why does Hydra's `ToMap()` for ID token claims `delete` empty values rather than serializing them as empty strings?
...
### Q: Why is `amr` (Authentication Methods References) an array, not a single value?
...
### Q: What is the danger of using a raw OAuth2 access token as proof of login without an ID token?
...
### Q: What is refresh token rotation, and why is it only half the mechanism without reuse detection?
...
→ Post: `_posts/2025-11-19-refresh-tokens-rotation-and-revocation.md`

### Q: Why must the server revoke the entire token family when reuse is detected, rather than just rejecting the replayed token?
...
### Q: Why do `RotateRefreshToken` and `CreateAccessTokenSession` happen in the same database transaction?
...
### Q: What does `handleRefreshTokenReuse` revoking by `req.GetID()` instead of the specific token signature mean?
...
### Q: Why are refresh tokens arguably higher-value targets than access tokens?
...
### Q: What is `ErrInactiveToken` in Hydra's refresh flow, and how is it different from `ErrInvalidGrant`?
...
### Q: When does RBAC break down and ABAC become necessary?
...
→ Post: `_posts/2025-11-21-rbac-vs-abac-authorization-models.md`

### Q: What is the real integration cost difference between RBAC and ABAC at enforcement time?
...
### Q: What does Casbin's `g(r.sub, p.sub)` matcher do that a simple string equality check cannot?
...
### Q: Why does Casbin's ABAC model have no `[role_definition]` section and no policy CSV?
...
### Q: Are RBAC and ABAC mutually exclusive in production?
...
### Q: What does the ABAC matcher `r.sub == r.obj.Owner` assume about the request object that RBAC does not?
...
### Q: Why is SHA-256 the right hash choice for API keys but the wrong one for password hashing?
...
→ Post: `_posts/2025-11-23-api-key-authentication.md`

### Q: Why is the plaintext API key shown exactly once and never stored?
...
### Q: Why is a plain equality lookup (`WHERE k.hash = ?`) safe for API keys but would be dangerous for password hashes?
...
### Q: Why does Unkey use base58 encoding for API keys instead of base64?
...
### Q: What is the purpose of the unhashed `start` field stored alongside the hash?
...
### Q: What is the common mistake of applying password hashing best practices to API key storage?
...
### Q: Why doesn't mTLS with SPIFFE/SPIRE use hostname or IP for identity verification?
...
→ Post: `_posts/2025-11-25-mtls-certificate-based-service-auth.md`

### Q: Why does `ValidateCSR` reject certificate signing requests with zero or more than one URI SAN?
...
### Q: What is the difference between `AuthorizeID` and `AuthorizeMemberOf` in go-spiffe?
...
### Q: Why is mTLS structurally superior to shared secrets for service-to-service authentication?
...
### Q: Why must the `Authorizer` function check the peer's identity rather than the network address the connection was made through?
...
### Q: What common assumption about traditional TLS verification is wrong in the SPIFFE/SPIRE model?
...
### Q: What makes SAML assertion verification harder than JWT verification?
...
→ Post: `_posts/2025-11-27-sso-and-saml-assertion-verification.md`

### Q: Why does SAML validation have a two-sided time window (NotBefore and NotOnOrAfter) while JWT typically only checks expiry?
...
### Q: What is a SAML signature-wrapping attack and how does Keycloak defend against it?
...
### Q: Why do both the SAML issuing side and verifying side apply clock-skew adjustments in opposite directions?
...
### Q: Why is SAML still found in enterprise environments despite OIDC being the modern default?
...
### Q: Why does Keycloak log `assertion.getID()` on expiry failure rather than a generic error message?
...
### Q: Why is checking only "is this TOTP code mathematically valid for the current time window" insufficient?
...
→ Post: `_posts/2025-11-29-mfa-totp-webauthn-passkeys.md`

### Q: How does WebAuthn detect a cloned authenticator without sharing the private key?
...
### Q: Why is TOTP step history keyed by `step*config.Period` rather than the raw 6-digit code?
...
### Q: What is the difference between WebAuthn used as a second factor and a passkey?
...
### Q: Why is `CloneWarning` a property Authelia checks rather than computes itself?
...
### Q: What is the vulnerability of TOTP without step-reuse tracking, and how prevalent is it in real implementations?
...
### Q: What is the core difference between an identity check and an authorization decision?
...
### Q: Why is broken access control resistant to automated security scanning compared to SQL injection or XSS?
...
### Q: How does WebGoat's IDOR example demonstrate privilege escalation beyond data theft?
...
### Q: What do JWT algorithm confusion, session fixation, and broken access control have in common as root causes?
...
### Q: What does the `if` condition in WebGoat's vulnerable code do that it shouldn't?
...
### Q: Why has broken access control risen to OWASP #1, and what makes it structurally persistent?
